...
type EventData = {
  ...
  category: string;
  image?: typeof onchainevent; // Using the specific image type
  price?: number;
  organizer: string;
};
...
export function Event({ setActiveTab }: HomeProps) {
  ...
  return (
    <div className="space-y-4 animate-fade-in w-full h-full z-10 fixed top-0 right-0 p-4 overflow-y-auto bg-white min-h-full">
      {/* Header */}
      <header className="flex items-center justify-between w-full bg-red-0 backdrop-blur-sm rounded-none p-4 shadow-sm">
        <div className="flex items-center space-x-3">
          ...
          <div>
            <h1 className="text-xl font-bold text-[var(--app-foreground)]">Base Onchain Summer III Workshop - Accra</h1>
            <p className="text-sm text-[var(--app-foreground-muted)]">Confirm your participation</p>
          </div>
        </div>
        <div className="text-right">
          <div className="text-sm text-[var(--app-foreground-muted)]">Category</div>
          <div className="text-sm font-medium text-[var(--app-accent)]">{mockEvent.category}</div>
        </div>
      </header>

      {/* Event Image */}
      {/* <div */}
      <Image src={onchainevent} alt="Event Banner"
        className="w-full bg-gradient-to-br from-blue-500 to-purple-600 rounded-xl flex items-center justify-center mt-50 overflow-hidden mb-50" py-10 />
      {/* </div> */}

      <div className="gap-2 flex justify-center items-center mx-auto px-5 my-5">
        <div className="w-full flex-1 h-[5rem] bg-gray-100 rounded-md aspect-square"></div>
        <div className="w-full flex-1 h-[5rem] bg-gray-100 rounded-md aspect-square"></div>
        <div className="w-full flex-1 h-[5rem] bg-gray-100 rounded-md aspect-square"></div>
        <div className="w-full flex-1 h-[5rem] bg-gray-100 rounded-md aspect-square"></div>
      </div>
      ...
      {/* Event Details */}
      <Card title="" className="border-0 shadow-none hover:shadow-none">
        <div className="space-y-4">
          <div className="flex items-start space-x-3">
            <Icon name="calendar" className="text-[var(--app-accent)] mt-1" />
            <div>
              <div className="font-medium text-[var(--app-foreground)]">{mockEvent.date}</div>
              <div className="text-sm text-[var(--app-foreground-muted)]">{mockEvent.time}</div>
            </div>
          </div>

          <div className="flex items-start space-x-3">
            <Icon name="map-pin" className="text-[var(--app-accent)] mt-1" />
            <div>
              <div className="font-medium text-[var(--app-foreground)]">Location</div>
              <div className="text-sm text-[var(--app-foreground-muted)]">{mockEvent.location}</div>
            </div>
          </div>

          <div className="flex items-start space-x-3">
            <Icon name="users" className="text-[var(--app-accent)] mt-1" />
            <div>
              <div className="font-medium text-[var(--app-foreground)]">Organized by</div>
              <div className="text-sm text-[var(--app-foreground-muted)]">{mockEvent.organizer}</div>
            </div>
          </div>
              ...
              <div className="mt-4 p-4 bg-gray-50 border border-gray-200 rounded-lg">
                ...
                <div className="flex space-x-2">
                  <input
                    ...
                    className="flex-1 px-3 py-2 border outline-none border-gray-300 rounded-lg text-sm uppercase "
                  />
                  <Button
                    size="sm"
                    onClick={() => {
      ...
      {showConfirmation && (
        <Card title="Participation Confirmed!">
